...
2. В `CHANGELOG.md` инструмента (`Container image optimizer/CHANGELOG.md` или
   `SQL backup restore clone/CHANGELOG.md`) в заголовке раздела `## [X.Y.Z] - Unreleased` заменить
   пометку `Unreleased` на дату выпуска (например, `## [0.1.0] - 2026-10-01`), сразу завести новую
   секцию следующей версии с пометкой `- Unreleased` и ссылки сравнения в конце файла поправить
   на теги того же инструмента.
...
6. Заведите `CHANGELOG.md` в каталоге инструмента (раздел `## [X.Y.Z] - Unreleased`, где `X.Y.Z` —
   версия следующего релиза, а дата подставляется при выпуске), добавьте модуль
