...
Версии у инструментов независимые, поэтому раздел `CHANGELOG.md` называется `<инструмент> X.Y.Z`
(`cio 0.2.0`, `sqlbrc 0.1.0`), а ссылки сравнения ведут на теги того же инструмента: заголовки вида
`## [0.1.0]` в мультипроектном файле не различить.
...
2. В `CHANGELOG.md` переименовать `## [Unreleased]` в `## [<инструмент> X.Y.Z] - ГГГГ-ММ-ДД`
   (например, `## [sqlbrc 0.1.0] - 2026-10-01`), сразу добавить новый пустой `## [Unreleased]`
...
4. Обновите таблицу инструментов в корневом `README.md` и раздел `## [Unreleased]` в `CHANGELOG.md`.
