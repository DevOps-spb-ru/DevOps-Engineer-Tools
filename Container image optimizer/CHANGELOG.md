...
- образ `cio` собирался без коммита и даты сборки: `Dockerfile` подставлял только `main.version`,
  а `release-image.yml` не передавал `--build-arg DATE`, поэтому `cio --version` внутри образа
  печатал «commit none, сборка unknown», и сверить контейнер с бинарём из релиза было нечем.
  Теперь значения подставляются так же, как в релизных бинарях.

### Изменилось

- минимальная версия Go — `1.26.8` (директива `go` в `go.mod` и `ARG GO_IMAGE` в Dockerfile): в образе
